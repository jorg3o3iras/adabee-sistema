// ============================================
    ...
    try {
        ...
        if (tbody) {
            ...
            // ============================================
            // ============================================
// SALVAR CORREÇÃO MANUAL - VERSÃO MÍNIMA
// ============================================
function salvarCorrecaoManualStandalone() {
    ...
    if (!temResposta) { 
        showToast('⚠️ Marque pelo menos uma resposta!', 'warning'); 
        return; 
    }
    ...
    console.log('📤 Enviando:', JSON.stringify(dadosCorrecao));
    ...
    .then(r => r.json().catch(() => ({ erro: 'Erro ao processar resposta' })))
    .then(data => {
        if (data.sucesso) {
            showToast(`✅ Salvo! Nota: ${nota.toFixed(1)}`, 'success');
        ...
        console.error(erro);
            ...
            async function carregarRelatorioTurmaFiltrado() {
                try {
                    ...
                    // ============================================
                    // 🔥 ATUALIZAR O CABEÇALHO DA TABELA - APENAS UMA DISCIPLINA (IGUAL AO EXEMPLO)
                    // ============================================
                    ...
                    // 🔥 TABELA COM APENAS A DISCIPLINA SELECIONADA - IGUAL AO EXEMPLO
...
function exportarRelatorioPDF() {
    ...
    // 🔥 SE OS TOTAIS ESTÃO ZERADOS, TENTA PEGAR DOS CARDS
    if (totalAcertos === 0 && acertosPorQuestao.length > 0) {
        acertosPorQuestao.forEach(q => {
            totalAcertos += parseInt(q.acertos) || 0;
 ...
 // ============================================
// 🔥 FUNÇÃO PARA IMPRIMIR RESULTADOS MELHORADOS
// ============================================
function imprimirResultadosMelhorados() {
    const tbody = document.getElementById('tb-resultados-filtrado');
    if (!tbody) {
    ...
    const rows = tbody.querySelectorAll('tr');
    if (rows.length === 0 || (rows.length === 1 && rows[0].textContent.includes('Nenhum'))) {
        showToast('⚠️ Nenhum dado disponível para impressão.', 'error');
        return;
    }

    // Obtém os filtros atuais
    const escolaSelect = document.getElementById('filtro-escola');
    const serieSelect = document.getElementById('filtro-serie');
    const turmaSelect = document.getElementById('filtro-turma');
    const provaSelect = document.getElementById('filtro-prova');

    const escolaNome = escolaSelect.options[escolaSelect.selectedIndex]?.text || 'Todas as escolas';
    const serieNome = serieSelect.options[serieSelect.selectedIndex]?.text || 'Todas as séries';
    const turmaNome = turmaSelect.options[turmaSelect.selectedIndex]?.text || 'Todas as turmas';
    const provaNome = provaSelect.options[provaSelect.selectedIndex]?.text || 'Todas as provas';
    ...
    rows.forEach(row => {
        ...
        if (cells.length >= 20) {
            ...
            // Português
            const ptAcertos = parseInt(cells[4]?.textContent.trim()) || 0;
            const ptErros = parseInt(cells[5]?.textContent.trim()) || 0;
            const ptConceito = cells[6]?.textContent.trim() || '';
            
            // Matemática
            const matAcertos = parseInt(cells[7]?.textContent.trim()) || 0;
            const matErros = parseInt(cells[8]?.textContent.trim()) || 0;
            const matConceito = cells[9]?.textContent.trim() || '';
            
            // Produção
            const prodAcertos = parseInt(cells[10]?.textContent.trim()) || 0;
            const prodErros = parseInt(cells[11]?.textContent.trim()) || 0;
            const prodConceito = cells[12]?.textContent.trim() || '';
            
            // CH
            const chAcertos = parseInt(cells[13]?.textContent.trim()) || 0;
            const chErros = parseInt(cells[14]?.textContent.trim()) || 0;
            const chConceito = cells[15]?.textContent.trim() || '';
            
            // CN
            const cnAcertos = parseInt(cells[16]?.textContent.trim()) || 0;
            const cnErros = parseInt(cells[17]?.textContent.trim()) || 0;
            const cnConceito = cells[18]?.textContent.trim() || '';
            
            const escola = cells[19]?.textContent.trim() || '';
            const turma = cells[20]?.textContent.trim() || '';

            // Soma os totais da disciplina para o resumo
            const totalAcertosAluno = ptAcertos + matAcertos + prodAcertos + chAcertos + cnAcertos;
            const totalErrosAluno = ptErros + matErros + prodErros + chErros + cnErros;
            const totalQuestoesAluno = (ptAcertos + ptErros) + (matAcertos + matErros) + 
                                       (prodAcertos + prodErros) + (chAcertos + chErros) + 
                                       (cnAcertos + cnErros);

            totalAcertosGeral += totalAcertosAluno;
            totalErrosGeral += totalErrosAluno;
            totalQuestoesGeral += totalQuestoesAluno;
            
            alunosData.push({ 
                posicao, numero, nome, serie, 
                ptAcertos, ptErros, ptConceito,
                matAcertos, matErros, matConceito,
                prodAcertos, prodErros, prodConceito,
                chAcertos, chErros, chConceito,
                cnAcertos, cnErros, cnConceito,
                escola, turma
            });
        }
    });
    ...
    // ============================================
    // CÁLCULO DOS INDICADORES DA TURMA
    // ============================================
    const totalAlunos = alunosData.length;
    
    // Média de acertos da turma (em porcentagem)
    let mediaTurma = 0;
    if (totalAlunos > 0 && totalQuestoesGeral > 0) {
        mediaTurma = Math.round((totalAcertosGeral / totalQuestoesGeral) * 100);
    }

    // Conceito da Turma baseado na média
    let conceitoTurma = '';
    let conceitoCor = '';
    if (mediaTurma <= 40) {
        conceitoTurma = 'Inicial';
        conceitoCor = '#ef4444';
    } else if (mediaTurma <= 60) {
        conceitoTurma = 'Básico';
        conceitoCor = '#f59e0b';
    } else if (mediaTurma <= 80) {
        conceitoTurma = 'Proficiente';
        conceitoCor = '#3b82f6';
    } else {
        conceitoTurma = 'Avançado';
        conceitoCor = '#10b981';
    }
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .resumo-turma .item .valor {
                font-size: 22px;
                font-weight: 900;
            }
            
            .resumo-turma .item .valor.acertos { color: #10b981; }
            .resumo-turma .item .valor.erros { color: #ef4444; }
            .resumo-turma .item .valor.media { color: #3b82f6; }
            
            .resumo-turma .item .label {
                font-size: 8px;
                color: #94a3b8;
                font-weight: 600;
                text-transform: uppercase;
            ...
            .resumo-turma .item .conceito-badge {
                display: inline-block;
                padding: 4px 16px;
                border-radius: 20px;
                font-size: 14px;
            ...
            .badge-inicial-sm { background: #fef2f2; color: #ef4444; }
            .badge-basico-sm { background: #fffbeb; color: #f59e0b; }
            .badge-proficiente-sm { background: #eff6ff; color: #3b82f6; }
            .badge-avancado-sm { background: #ecfdf5; color: #10b981; }
            
            .col-disciplina-header {
                background: rgba(0,0,0,0.02);
                border-bottom: 2px solid;
            }
            
            .col-pt { border-bottom-color: #3b82f6; }
            .col-mat { border-bottom-color: #10b981; }
            .col-prod { border-bottom-color: #8b5cf6; }
            .col-ch { border-bottom-color: #f59e0b; }
            .col-cn { border-bottom-color: #14b8a6; }
            
            .acertos-text { color: #10b981; font-weight: 700; }
            .erros-text { color: #ef4444; font-weight: 700; }
            ...
            @media (max-width: 768px) {
                .filtros-info { grid-template-columns: repeat(2, 1fr); }
                .resumo-turma { grid-template-columns: repeat(2, 1fr); }
                .assinaturas { grid-template-columns: 1fr; gap: 16px; }
                .header-top { flex-direction: column; align-items: flex-start; }
                .header-right { border-left: none; padding-left: 0; text-align: left; }
            }
        </style>
    ...
    <body>
        ...
        <div class="header">
            <div class="header-top">
                <div class="header-left">
                    <img src="https://github.com/jorg3o3iras/adabee-sistema/blob/main/dgp.jpg?raw=true" alt="DGP" class="logo" style="width:50px;height:50px;object-fit:contain;border-radius:4px;">
                    <div class="instituicao">
                        <span class="prefeitura">PREFEITURA MUNICIPAL DE</span>
                        <span class="secretaria">SÃO SEBASTIÃO DA BOA VISTA</span>
                        <span class="departamento">SECRETARIA MUNICIPAL DE EDUCAÇÃO</span>
                ...
                <div class="header-right">
                    <div class="titulo-relatorio">📊 RELATÓRIO DE RESULTADOS</div>
                    <div class="subtitulo-relatorio">Relatório de desempenho dos alunos por disciplina</div>
        ...
        <div class="filtros-info">
            <div class="item">
                ...
                <span class="value">${escolaNome}</span>
            </div>
            <div class="item">
                <span class="label">📚 Série</span>
                <span class="value">${serieNome}</span>
            </div>
            <div class="item">
                <span class="label">👥 Turma</span>
                <span class="value">${turmaNome}</span>
            </div>
            <div class="item">
                <span class="label">📄 Prova</span>
                <span class="value">${provaNome}</span>
            </div>
        </div>

        <!-- ===== RESUMO DA TURMA ===== -->
        <div class="resumo-turma">
            <div class="item">
                <div class="valor acertos">${totalAcertosGeral}</div>
                <div class="label">✅ Total de Acertos</div>
            </div>
            <div class="item">
                <div class="valor erros">${totalErrosGeral}</div>
                <div class="label">❌ Total de Erros</div>
            </div>
            <div class="item">
                <div class="valor media">${mediaTurma}%</div>
                <div class="label">📊 Média da Turma</div>
            </div>
            <div class="item">
                <div class="conceito-badge">${conceitoTurma}</div>
                <div class="label">📊 Conceito da Turma</div>
            </div>
        </div>

        <!-- ===== TOTAL DE ALUNOS ===== -->
        <div class="total-alunos-info">
            Total de Alunos na Lista: <strong>${totalAlunos}</strong>
        </div>
            ...
            <table>
                <thead>
                    <tr>
                        <th rowspan="2">Pos.</th>
                        <th rowspan="2">Nº</th>
                        <th rowspan="2" style="text-align:left;">Nome</th>
                        <th rowspan="2">Série</th>
                        <th colspan="3" class="col-disciplina-header col-pt" style="border-bottom:2px solid #3b82f6;font-size:6px;">Português</th>
                        <th colspan="3" class="col-disciplina-header col-mat" style="border-bottom:2px solid #10b981;font-size:6px;">Matemática</th>
                        <th colspan="3" class="col-disciplina-header col-prod" style="border-bottom:2px solid #8b5cf6;font-size:6px;">Produção</th>
                        <th colspan="3" class="col-disciplina-header col-ch" style="border-bottom:2px solid #f59e0b;font-size:6px;">CH</th>
                        <th colspan="3" class="col-disciplina-header col-cn" style="border-bottom:2px solid #14b8a6;font-size:6px;">CN</th>
                        <th rowspan="2" style="font-size:6px;">Escola</th>
                        <th rowspan="2" style="font-size:6px;">Turma</th>
                    </tr>
                    <tr>
                        <th style="font-size:5px;">Acertos</th>
                        <th style="font-size:5px;">Erros</th>
                        <th style="font-size:5px;">Conceito</th>
                        <th style="font-size:5px;">Acertos</th>
                        <th style="font-size:5px;">Erros</th>
                        <th style="font-size:5px;">Conceito</th>
                        <th style="font-size:5px;">Acertos</th>
                ...
                <tbody>
                    ${alunosData.map((a, index) => {
                        const medalha = a.posicao === '🥇' ? '🥇' : 
                                       a.posicao === '🥈' ? '🥈' : 
                                       a.posicao === '🥉' ? '🥉' : 
                                       a.posicao || (index + 1);
                        
                        // Função para badge de conceito
                        const badgeConceito = (conceito) => {
                            const classes = {
                                'inicial': 'badge-inicial-sm',
                                'basico': 'badge-basico-sm',
                                'proficiente': 'badge-proficiente-sm',
                                'avancado': 'badge-avancado-sm'
                            };
                            const cls = classes[conceito?.toLowerCase()] || 'badge-inicial-sm';
                            return `<span class="badge-conceito-sm ${cls}">${conceito || '—'}</span>`;
                        };
                        
                        return `
                            <tr>
                                <td><span style="font-size:12px;">${medalha}</span></td>
                                <td>${a.numero}</td>
                                <td class="nome-aluno">${a.nome}</td>
                                <td>${a.serie}</td>
                                <td><span class="acertos-text">${a.ptAcertos}</span></td>
                                <td><span class="erros-text">${a.ptErros}</span></td>
                                <td>${badgeConceito(a.ptConceito)}</td>
                                <td><span class="acertos-text">${a.matAcertos}</span></td>
                                <td><span class="erros-text">${a.matErros}</span></td>
                                <td>${badgeConceito(a.matConceito)}</td>
                                <td><span class="acertos-text">${a.prodAcertos}</span></td>
                                <td><span class="erros-text">${a.prodErros}</span></td>
                                <td>${badgeConceito(a.prodConceito)}</td>
                                <td><span class="acertos-text">${a.chAcertos}</span></td>
                                <td><span class="erros-text">${a.chErros}</span></td>
                                <td>${badgeConceito(a.chConceito)}</td>
                                <td><span class="acertos-text">${a.cnAcertos}</span></td>
                                <td><span class="erros-text">${a.cnErros}</span></td>
                                <td>${badgeConceito(a.cnConceito)}</td>
                                <td class="col-escola">${a.escola}</td>
                                <td class="col-turma">${a.turma}</td>
                            </tr>
                        `;
        ...
        <div class="assinaturas">
            <div class="assinatura-item">
                <div class="linha"></div>
                <div class="nome-assinatura">Professor(a) Responsável</div>
                <div class="cargo">Professor(a)</div>
            </div>
            <div class="assinatura-item">
                <div class="linha"></div>
                <div class="nome-assinatura">Diretor(a)</div>
                <div class="cargo">Diretor(a)</div>
            </div>
            <div class="assinatura-item">
                <div class="linha"></div>
                <div class="nome-assinatura">Secretário(a) de Educação</div>
                <div class="cargo">Secretaria Municipal de Educação</div>
            </div>
        </div>
    ...
    showToast('🖨️ Relatório enviado para impressão!', 'success');
}

            // ============================================
            // 🔥 FUNÇÃO PARA IMPRIMIR DESEMPENHO MELHORADO
            // ============================================
            function imprimirDesempenhoMelhorado() {
                const data = desempenhoData.alunoSelecionado;
                if (!data) {
                    showToast('⚠️ Gere o desempenho primeiro!', 'error');
                    return;
                ...
                // Função para obter a badge do conceito
                function getConceitoBadge(conceito) {
                    const conceitos = {
                        'Inicial': { label: 'Inicial', color: '#ef4444', bg: '#fef2f2' },
                        'Básico': { label: 'Básico', color: '#f59e0b', bg: '#fffbeb' },
                        'Proficiente': { label: 'Proficiente', color: '#3b82f6', bg: '#eff6ff' },
                        'Avançado': { label: 'Avançado', color: '#10b981', bg: '#ecfdf5' }
                    };
                    const c = conceitos[conceito] || { label: conceito, color: '#64748b', bg: '#f1f5f9' };
                    return `<span style="background:${c.bg}; color:${c.color}; padding:2px 12px; border-radius:12px; font-size:10px; font-weight:700;">${c.label}</span>`;
                }
                ...
                <head>
                    ...
                    <style>
                        * { margin: 0; padding: 0; box-sizing: border-box; }
                        ...
                        .questao-item.acertou .q-status-text { color: #10b981; }
                        .questao-item.errou .q-status-text { color: #ef4444; }
                        ...
                        .gabarito-item .q-resp {
                            font-weight: 700;
                            font-size: 10px;
                            color: #10b981;
                        }
                        
                        .gabarito-item .q-bncc {
                            font-size: 5px;
                            color: #8b5cf6;
                            margin-top: 2px;
                        }
                        ...
                        @media (max-width: 768px) {
                            ...
                            .questoes-grid { grid-template-columns: repeat(5, 1fr); }
                            .gabarito-grid { grid-template-columns: repeat(5, 1fr); }
                            .assinaturas { grid-template-columns: 1fr; gap: 16px; }
                            .header-top { flex-direction: column; align-items: flex-start; }
                            .header-right { border-left: none; padding-left: 0; text-align: left; }
                        }
                ...
                <body>
                    ...
                    <div class="header">
                        <div class="header-top">
                            ...
                            <div class="header-right">
                                <div class="titulo-relatorio">📊 RELATÓRIO DE DESEMPENHO</div>
                                <div class="subtitulo-relatorio">${prova.titulo || 'Avaliação'}</div>
                                <div style="font-size:8px; color:#94a3b8; margin-top:2px;">
                                    ${disciplina} - Ano Letivo 2026
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="info-grid">
                        <div class="item">
                            <span class="label">🏫 Escola</span>
                            <span class="value">${escola.nome || '—'}</span>
                        </div>
                        <div class="item">
                            <span class="label">👥 Turma</span>
                            <span class="value">${(turma.nome || '') + ' - ' + (turma.serie || '')}</span>
                        </div>
                        <div class="item">
                            <span class="label">🎒 Aluno</span>
                            <span class="value">${aluno.nome || '—'}</span>
                        </div>
                        <div class="item">
                            <span class="label">📄 Prova</span>
                            <span class="value">${prova.titulo || '—'}</span>
                        </div>
                        <div class="item">
                        ...
                        <div class="item">
                            ...
                            <span class="value">${disciplina}</span>
                        </div>
                    </div>
                    ...
                    <div class="resumo-grid">
                        <div class="resumo-card">
                            <div class="valor blue">${porcentagem}%</div>
                            <div class="label">📊 Porcentagem de Acertos</div>
                        </div>
                        <div class="resumo-card">
                            <div class="valor green">${acertos}</div>
                            <div class="label">✅ Acertos</div>
                        </div>
                        <div class="resumo-card">
                            <div class="valor red">${erros}</div>
                            <div class="label">❌ Erros</div>
                        </div>
                        <div class="resumo-card">
                            <div class="valor purple">${getConceitoBadge(conceito)}</div>
                    ...
                    <div class="section-title">
                        📖 ${disciplina}
                        <span class="badge">${totalQuestoes} questões</span>
                    </div>
                    ...
                    <div class="questoes-grid">
                        ${resultadoQuestoes.map(q => {
                            ...
                            const resposta = q.resposta || '—';
                            const gab = q.gabarito || '—';
                            const statusIcon = acertou ? '✅' : (q.respondida ? '❌' : '—');
                            const statusText = acertou ? 'ACERTOU' : (q.respondida ? 'ERROU' : 'NÃO RESP.');
                            const statusClass = acertou ? 'acertou' : (q.respondida ? 'errou' : '');
                            const bnccHtml = q.bncc ? `<div style="font-size:5px; color:#8b5cf6; margin-top:1px;">BNCC: ${q.bncc}</div>` : '';
                            return `
                                <div class="questao-item ${statusClass}">
                                    <div class="q-num">Q${q.numero}</div>
                                    <div class="q-resp">${resposta}</div>
                                    <div class="q-gab">${gab}</div>
                                    <span class="q-status-text">${statusIcon} ${statusText}</span>
                                    ${bnccHtml}
                                </div>
                            `;
                        }).join('')}
                    ...
                    <div class="gabarito-grid">
                        ${gabarito.map((gab, idx) => {
                            const codigoBncc = (idx < bncc.length) ? bncc[idx] : '';
                            const bnccHtml = codigoBncc ? `<div class="q-bncc">BNCC: ${codigoBncc}</div>` : '';
                            return `
                                <div class="gabarito-item">
                                    <div class="q-num">Q${idx+1}</div>
                                    <div class="q-resp">${gab || '—'}</div>
                ...
                showToast('🖨️ Relatório de desempenho enviado para impressão!', 'success');
            }
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                /* LISTA VERTICAL - UMA QUESTÃO POR LINHA */
                .questoes-container {
                    display: flex;
                    flex-direction: column;
                    gap: 2px;
                ...
                @media print {
                    ...
                    /* Garante que tudo caiba em uma página */
